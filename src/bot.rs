    ...
    match error {
        ...
        poise::FrameworkError::ArgumentParse {
                ...
                .is_some()
            {
                ...
                match input {
                    Some(input) => {
                        format!("Couldn't understand `{input}` for `{prefix}{invoked}`: {error}")
                    }
                    None => format!("Couldn't parse that for `{prefix}{invoked}`: {error}"),
                }
            };

            let reply = poise::CreateReply::default().content(body).ephemeral(true);
            let _ = ctx.send(reply).await;
        }
        error => {
